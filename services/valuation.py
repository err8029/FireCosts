...
def _footprint_area_m2(geom):
    lat = geom.centroid.y
    m_per_deg_lon = _M_PER_DEG_LAT * math.cos(math.radians(lat))
    return geom.area * _M_PER_DEG_LAT * m_per_deg_lon


def _fallback_result(osm_id, geom, rc, address, municipality, default_price, source="osm_footprint_estimate"):
    area_m2 = _footprint_area_m2(geom)
    return {
        "osm_id": osm_id,
        "rc": rc,
        "address": address,
...
def _lookup_building(feature, default_price, rate_limited_flag):
    geom = shape(feature["geometry"])
    props = feature.get("properties", {})
    osm_id = props.get("osm_id")
    osm_city = props.get("addr_city")
    ...
    if rate_limited_flag.is_set():
        return _fallback_result(osm_id, geom, None, None, osm_city, default_price, source="catastro_rate_limited")
    ...
    try:
        ...
        return _fallback_result(osm_id, geom, None, None, osm_city, default_price, source="catastro_error")

    if not rc:
        return _fallback_result(osm_id, geom, None, None, osm_city, default_price)

    try:
        details = catastro.lookup_details(rc)
    except catastro.CatastroRateLimitError:
        rate_limited_flag.set()
        return _fallback_result(osm_id, geom, rc, address, osm_city, default_price, source="catastro_rate_limited")
    except Exception:
        logger.warning("Catastro lookup_details failed for osm_id=%s rc=%s", osm_id, rc, exc_info=True)
        return _fallback_result(osm_id, geom, rc, address, osm_city, default_price, source="catastro_error")

    if not details or not details.get("total_built_area_m2"):
        return _fallback_result(osm_id, geom, rc, address, osm_city, default_price)
    ...
    return {
        ...
        "municipality": details.get("municipality") or osm_city or "Others",
...
def estimate_value_lost(buildings_geojson, default_price_per_m2=DEFAULT_PRICE_PER_M2_EUR):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = [
            pool.submit(_lookup_building, f, default_price_per_m2, rate_limited_flag) for f in features
        ]
